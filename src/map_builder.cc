...
void MapBuilder::ExtractFeatureThread(){
  while(!_shutdown || !_data_buffer.empty()){
    ...
    if(!_init || _insert_next_keyframe){
      ...
      BboxPtr bboxs_left = _feature_detector->YoloDetect(image_left_rect, left_features);
      std::cout << "left_features size: " << left_features.cols() << std::endl;
      std::cout << "bboxs_left size: " << bboxs_left->size() << std::endl;
      // 获取第二次检测结果
      BboxPtr bboxs_right = _feature_detector->YoloDetect(image_right_rect, right_features);
      std::cout << "right_features size: " << right_features.cols() << std::endl;
      std::cout << "bboxs_right size: " << bboxs_right->size() << std::endl;
      std::vector<tensorrt_buffer::BBox> _merged_bboxs;
      // 将第一次检测结果添加到合并后的 vector 中
      if (bboxs_left) {
          _merged_bboxs.insert(_merged_bboxs.end(), bboxs_left->begin(), bboxs_left->end());
      }

      // 将第二次检测结果添加到合并后的 vector 中
      if (bboxs_right) {
          _merged_bboxs.insert(_merged_bboxs.end(), bboxs_right->begin(), bboxs_right->end());
      }
      // 创建一个新的 BboxPtr 指向合并后的 vector
      BboxPtr bboxs = std::make_shared<std::vector<tensorrt_buffer::BBox>>(_merged_bboxs);
      std::cout << "merged_bboxs size: " << bboxs->size() << std::endl;
      frame->SetBboxs(bboxs);
...
void MapBuilder::PublishFrame(FramePtr frame, cv::Mat& image, FrameType frame_type, std::vector<cv::DMatch>& matches){
  ...
  if (frame->GetBboxs()->size() > 0) {
    std::cout << "feature_message bboxs size: " << frame->GetBboxs()->size() << std::endl;
    feature_message->bboxs = *(frame->GetBboxs()); 
  }
